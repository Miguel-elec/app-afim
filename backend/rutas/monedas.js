...
function monedasAPI(app) {
    ...
    router.get('/:monedaId', async function (req, res, next) {
        const {monedaId}  = req.params;  // const monedaId = req.params.monedaId;
        console.log(`buscando operarion ${monedaId}`);
        try {
            const moneda = await monedasService.getMoneda(monedaId);
            res.status(200).json({
                data: moneda,
                message: 'operacion devuelta con éxito'
            });
        } catch (err) {
            next(err);
        }
    ...
    router.post('/', async function (req, res, next) {
        const { body: moneda } = req; // body = req.body; moneda = body  / los : definen un alias
        try {
